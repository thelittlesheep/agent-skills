...
install_codex() {
  ...
  for skill_dir in "$SCRIPT_DIR"/plugins/*/skills/*/; do
    ...
    ln -sf "$skill_dir" "$HOME/.codex/skills/$name"
    ln -sf "$skill_dir" "$HOME/.agents/skills/$name"
  done
  shopt -u nullglob
  echo "  Codex: skills -> ~/.codex/skills/ & ~/.agents/skills/"
}
...
echo "Installing agent-marketplace..."
echo ""
build_tools || echo "  Warning: some tools failed to build"
install_config || echo "  Warning: config install failed"
echo ""

echo "Linking runtime assets..."
install_codex || echo "  Codex: skipped (error during install)"
install_opencode || echo "  OpenCode: skipped (error during install)"
echo ""

echo "Register with your AI runtime:"
echo "  Claude Code: claude plugin marketplace add $SCRIPT_DIR"
echo "               claude plugin install <plugin-name>"
echo "  Gemini CLI:  gemini extensions link $SCRIPT_DIR"
echo ""
echo "Done!"
