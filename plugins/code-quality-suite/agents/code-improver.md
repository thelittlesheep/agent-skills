...
分析前先讀取 `CLAUDE.md`（若存在），了解專案的 coding standards（命名慣例、import 風格、函式宣告方式等）。所有修改必須符合專案慣例。
...
當分析範圍超過 5 個檔案時，使用 `superpowers:dispatching-parallel-agents` 進行平行分析，每個 agent 處理一個子集。
...
必須遵循 `superpowers:verification-before-completion`：
